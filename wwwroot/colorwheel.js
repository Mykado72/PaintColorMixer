window.chromaMix = (() => {
    ...
    function tryDraw(img, dataUrl, dotNetRef, attempts) {
        ...
        if (_zCanvas) {
            _zCanvas.width = PZ; _zCanvas.height = PZ;
            _zCtx = _zCanvas.getContext('2d');
        }
        dotNetRef.invokeMethodAsync('OnPipetteImageReady', dataUrl);
    }

    function pipettePreview(x, y) {
        if (!_pCtx) return null;
        if (_zCtx && _zCanvas) {
            const half = PZ/2, src = PZ/PZF;
            _zCtx.clearRect(0, 0, PZ, PZ);
            _zCtx.save();
            _zCtx.beginPath(); _zCtx.arc(half, half, half-2, 0, Math.PI*2); _zCtx.clip();
            _zCtx.imageSmoothingEnabled = false;
            _zCtx.drawImage(_pCanvas, x-src/2, y-src/2, src, src, 0, 0, PZ, PZ);
            _zCtx.restore();
            _zCtx.beginPath(); _zCtx.arc(half, half, half-2, 0, Math.PI*2);
            ...
            _zCtx.beginPath();
            _zCtx.moveTo(half-10, half); _zCtx.lineTo(half+10, half);
            _zCtx.moveTo(half, half-10); _zCtx.lineTo(half, half+10);
            _zCtx.stroke(); _zCtx.shadowBlur = 0;
        }
        const px = _pCtx.getImageData(Math.round(x), Math.round(y), 1, 1).data;
    ...
    return {
        ...
        loadPipetteFromDataUrl, pipettePreview, getCanvasOffset,
